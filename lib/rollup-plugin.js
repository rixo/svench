...
import { defaults, defaultOutput } from './config'
...
const createPlugin = ({
  ...
  const start = async (outputOptions = {}) => {
    ...
    const script =
      publicDirs.length > 0
        ? '/' + findRelative(entryFile).replace(/\\/g, '/')
        : mountEntry

    const getIndex =
      indexCfg && indexCfg !== true
        ? await createIndex(indexCfg, { watch, script, onChange: hotReload })
        : _template({ script })

    if (serve && watch) {
      server = createServer(serve, { getIndex, entryFile, mountEntry })
    }
  }
