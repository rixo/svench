...
export default ({ preprocess, autoPage, autoComponentIndex }) => options => {
  ...
  const isAutoComponentIndex = file =>
    toArray(autoComponentIndex).some(ext => {
      if (!file.ext) {
        debugger
        console.trace(file)
        process.exit()
      }
      return file.ext.endsWith(ext)
    })

  const resolveConflict = (file, existing) => {
    let index = null
    if (isAutoComponentIndex(file)) {
      if (isAutoComponentIndex(existing)) return false
      index = file
    } else if (isAutoComponentIndex(existing)) {
      index = existing
    } else {
      return false
    }
  ...
  const parser = async (item, previous) => {
    ...
    if (item.path.endsWith('/index')) {
      // item.tree = false
      item.path = item.path.slice(0, -'/index'.length) || '/'
    }
