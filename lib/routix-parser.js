...
export default ({ preprocess, autoComponentIndex }) => {
  ...
  const parser = options => {
    ...
    return async (item, previous) => {
      ...
      if (/(\/(?:index|\.svench\.svench))$/.test(item.path)) {
        item.tree = false
        item.isIndex = true
      }
      ...
      if (item.isFile) {
        ...
        if (item.options && item.options.section) {
          ...
          if (dir && dirWithoutExt !== dir) {
          ...
          } else if (item.path !== '/' && item.path !== '/index') {
            ...
            // => likewise, /index needs to remain the root index
            //
